...
Набор покрывает проверки вебхука (секрет, IP, сверку статуса с кассой, идемпотентность повторной доставки) и гоняется в CI на Python 3.11 и 3.14.
...
Веб-панель слушает `127.0.0.1:5001` (адрес задаётся `WEB_HOST`/`WEB_PORT`). Наружу порт не открывается: панель админская и ходит по HTTP, поэтому доступ — через ssh-туннель с рабочей машины:

```bash
ssh -N -L 5001:127.0.0.1:5001 root@<YOUR_HOST> -p 2222
# затем в браузере: http://127.0.0.1:5001
```

Альтернатива — reverse proxy с TLS и аутентификацией; тогда панель всё равно остаётся на петле, а наружу смотрит только Nginx.
...
4. Запишите в `.env`:
   - `YUKASSA_SHOP_ID=...`
   - `YUKASSA_SECRET_KEY=...`
5. Сгенерируйте секрет адреса уведомлений и запишите в `.env`:
   - `WEBHOOK_SECRET_TOKEN=$(openssl rand -hex 32)`.
6. Настройте webhook URL через Nginx reverse proxy:
   - публично: `https://<YOUR_DOMAIN>/webhook/yookassa?token=<WEBHOOK_SECRET_TOKEN>`;
   - внутренний upstream: `127.0.0.1:8080`.

> YooKassa доставляет уведомления только на **HTTPS, порт 443 или 8443**. Пока перед ботом стоит Nginx на `80` без TLS, касса не сможет достучаться до эндпоинта — это отдельный незакрытый блокер продакшена.

## Безопасность вебхука

YooKassa **не подписывает** уведомления: в [документации](https://yookassa.ru/developers/using-api/webhooks) подлинность предлагается подтверждать статусом объекта и IP-адресом отправителя, никакого HMAC-заголовка касса не присылает. Поэтому эндпоинт защищён тремя проверками, которые выполняются именно в этом порядке; любая непройденная даёт `400`, запись в лог и **ноль изменений** в базе.

| Проверка | Что именно | Где реализована |
|---|---|---|
| Подпись адреса | `?token=` из URL сверяется с `WEBHOOK_SECRET_TOKEN` (SENSITIVE_KEYS) через `hmac.compare_digest` (сравнение за постоянное время, не `==`) | `bot/webhook_security.py::verify_signature` |
| IP отправителя | Адрес должен попадать в официальные подсети YooKassa (`185.71.76.0/27`, `185.71.77.0/27`, `77.75.153.0/25`, `77.75.156.11`, `77.75.156.35`, `77.75.154.128/25`, `2a02:5180::/32`) | `bot/webhook_security.py::is_yookassa_ip` |
| Статус в кассе | `GET /v3/payments/{id}` с Basic Auth `shop_id:secret_key`; если касса отдаёт другой статус или не знает платёж — уведомление отбрасывается | `bot/webhook_security.py::fetch_payment_status` |

Настоящий адрес отправителя берётся из `X-Real-IP`, и только когда соединение пришло с петли, то есть от нашего же Nginx (он перезаписывает этот заголовок). При прямом подключении заголовки игнорируются, поэтому подделать адрес снаружи нельзя.

**Откуда секреты.** `WEBHOOK_SECRET_TOKEN` и ключи `YUKASSA_*` живут только в `.env` и маскируются в веб-панели (`SENSITIVE_KEYS`). Пустой `WEBHOOK_SECRET_TOKEN` закрывает эндпоинт полностью: незаконфигуренный вебхук отвечает `400` на всё, а бот пишет об этом ошибку при старте. Перепроверка статуса включается сама, когда заполнены `YUKASSA_SHOP_ID` и `YUKASSA_SECRET_KEY`.

**Ротация.** Сгенерируйте новый токен (`openssl rand -hex 32`), запишите в `.env`, обновите URL уведомлений в кабинете YooKassa и перезапустите `shopbot`. Пока новый адрес не сохранён в кабинете, касса будет получать `400` и повторять доставку в течение суток — потерянных платежей не будет, но ротацию лучше делать в одно окно. Подробный порядок — в [docs/RUNBOOK.md](docs/RUNBOOK.md).
...
Порт `5001` наружу не открывается: панель слушает петлю, доступ — ssh-туннелем.
...
A: Убедитесь, что запущен `shopbot-web` и что поднят ssh-туннель на `5001` (панель слушает только `127.0.0.1`); пароль — `ADMIN_WEB_PASSWORD` в `.env` (не светить в чат).
